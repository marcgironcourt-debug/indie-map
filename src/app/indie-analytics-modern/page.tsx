...
function Breakdown({title, rows}: {title: string; rows: {label: string; count: number}[]}) {
  return <Panel title={title}>{rows.length ? <ul className="space-y-3 text-sm">{rows.map(row => <li className="flex justify-between gap-4" key={row.label}><span>{row.label}</span><strong>{number(row.count)}</strong></li>)}</ul> : empty}</Panel>;
}
  ...
  return <main className="h-screen overflow-y-auto bg-[#f4f5f1] p-4 text-slate-900 md:p-8 print:h-auto print:overflow-visible print:bg-white">
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      {!data ? <Panel title="Les données n’ont pas pu être chargées"><p>Les compteurs sont indisponibles, pas à zéro. Réessaie avec le bouton Actualiser.</p></Panel> : <>
        ...
        {filters.view === "accounts" && <>
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <Card title="Comptes créés au total" value={data.accountOverview.total} hint="Tous les comptes enregistrés depuis le lancement, indépendamment de la période choisie."/>
            <Card title="Nouveaux comptes" value={data.newAccounts} before={data.previousNewAccounts} hint={`${range.label}. Le filtre de trafic et l’appareil ne s’appliquent pas aux créations de compte.`}/>
            <Card title="Comptes actifs dans la période" value={data.trackedAccounts} hint="Comptes connectés ayant produit au moins une action pendant la période et avec les filtres sélectionnés."/>
            <Card title="Comptes avec e-mail" value={data.accountOverview.withEmail} hint="Total depuis le lancement. L’adresse reste visible uniquement dans ce dashboard privé."/>
            <Card title="Profils complétés" value={data.accountOverview.completedProfiles} hint="Comptes dont l’étape de profil a été terminée."/>
            <Card title="Notifications activées" value={data.accountOverview.pushEnabled} hint="Comptes liés à au moins un appareil ou une installation de notifications."/>
          </div>
          <div className="grid gap-6 lg:grid-cols-3">
            <Breakdown title="Villes déclarées" rows={data.accountCities}/>
            <Breakdown title="Tranches d’âge" rows={data.accountAges}/>
            <Breakdown title="Langues" rows={data.accountLocales}/>
          </div>
          <Panel title="Comptes utilisateurs">
            <div className="mb-4 flex flex-wrap items-start justify-between gap-3"><p className="max-w-3xl text-sm text-slate-500">Les 100 comptes les plus récents, avec leurs informations déclarées et leurs usages cumulés. Clique sur un nom pour ouvrir sa fiche technique détaillée.</p><p className="text-sm font-medium">Actifs sur 30 jours : {number(data.accountOverview.active30d)}</p></div>
            {data.accounts.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead><tr>{["Compte", "Créé", "Dernière activité", "Ville · âge", "Langue", "Favoris", "Visités", "Listes", "Actions", "Notifications"].map(label => <th className="p-3" key={label}>{label}</th>)}</tr></thead><tbody>{data.accounts.map(user => <tr className="border-t align-top" key={user.id}><td className="p-3"><a className="font-medium underline" href={`/indie-analytics?${new URLSearchParams({token,userId:user.id,tab:"users",scope:"total"})}`}>{user.displayName || user.username}</a><p className="mt-1 text-xs text-slate-500">@{user.username} · {user.email || "sans e-mail"}</p></td><td className="p-3">{new Date(user.createdAt).toLocaleDateString("fr-FR", {timeZone:"UTC"})}</td><td className="p-3">{user.lastSeenAt ? new Date(user.lastSeenAt).toLocaleDateString("fr-FR", {timeZone:"UTC"}) : "—"}</td><td className="p-3">{user.homeCity || "—"} · {user.ageRange || "—"}</td><td className="p-3">{user.preferredLocale || "—"}</td><td className="p-3">{user.favorites}</td><td className="p-3">{user.visited}</td><td className="p-3">{user.lists}</td><td className="p-3">{user.actions}</td><td className="p-3">{user.pushEnabled ? "Activées" : "Non reliées"}</td></tr>)}</tbody></table></div> : empty}
          </Panel>
          <p className="text-xs leading-relaxed text-slate-500">Les comptes, les visiteurs anonymes et les téléchargements des stores sont trois mesures différentes. Ce tableau ne transforme pas une installation ou une session en compte utilisateur.</p>
        </>}
