...
const heroOverlay = heroUrl
  ...
  const hoursTrim = String(openingHoursRaw || "").trim();
  const hoursPanelInner = hoursTrim
    ? hoursTrim.split(/\r?\n/).map(l => String(l||"").trim()).filter(Boolean)
        .map(l => "<div style=\"padding:2px 0;\">" + escapeHtml(l) + "</div>").join("")
    : "<div style=\"padding:2px 0; opacity:.75;\">Horaires : voir le site</div>";

  const hoursPanelHtml =
    "<div data-hours-panel=\"1\" data-open=\"0\" style=\"margin-top:8px; overflow:hidden; max-height:0px; opacity:0; transform:translateY(-6px); will-change:max-height,opacity,transform; transition:max-height 240ms cubic-bezier(0.22,1,0.36,1), opacity 200ms cubic-bezier(0.22,1,0.36,1), transform 240ms cubic-bezier(0.22,1,0.36,1);\" >" +
      "<div style=\"padding:10px 10px; border-radius:14px 6px 14px 6px; background:rgba(0,0,0,0.16); border:1px solid rgba(255,255,255,0.20); box-shadow:0 10px 22px rgba(0,0,0,0.18); font-family: ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial; font-size:11.5px; line-height:1.35; color:rgba(245,245,232,.90); letter-spacing:.02em;\" >" +
        hoursPanelInner +
      "</div>" +
    "</div>";
